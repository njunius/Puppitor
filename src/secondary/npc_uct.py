...
def uct_think(root_affect_vector, action_key_map, character_affecter, goal_emotion, itermax, rollout_max = 50):
    ...
    for iteration in range(itermax):
        ...
        if node.untried_moves:
            ...
            node = node.add_child(move, action_key_map, character_affecter.get_prevailing_affect(affect_vector))
        ...
        rollout_length = 0
        while affecter.evaluate_affect_vector(affect_vector, goal_emotion) < 0 and rollout_length < rollout_max:
        ...
        while node != None: # backpropogate from the expanded node towards the root node
            node.update(affecter.evaluate_affect_vector(affect_vector, goal_emotion))
