...
#
# the prevailing_states is broken into modifier states and actions
...
class Action_Key_Map:
    ...
    def __init__(self, key_map, default_action = 'resting', default_modifier = 'neutral'):
        ...
        for category in self.prevailing_states:
            if category == 'actions':
                self.prevailing_states['actions'][default_action] = True
                for action in self.key_map['actions']:
                    self.prevailing_states['actions'][action] = False
            
            if category == 'modifiers':
                self.prevailing_states['modifiers'][default_modifier] = True
                for modifier in self.key_map['modifiers']:
                    self.prevailing_states['modifiers'][modifier] = False
        ...
        for action in self.prevailing_states['actions'].keys():
            for modifier in self.prevailing_states['modifiers'].keys():
    ...
    def update_prevailing_states(self, state_to_update, class_of_action, new_value):
        
        updatable_states = self.prevailing_states[class_of_action]
